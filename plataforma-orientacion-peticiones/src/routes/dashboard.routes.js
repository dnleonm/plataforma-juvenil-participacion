...
router.get("/peticiones", verificarAutenticacion, async (req, res) => {
  try {
...
router.patch("/peticiones/:id/estado", verificarAutenticacion, async (req, res) => {
  ...
  try {
    ...
    const resultado = await cambiarEstadoPeticion(
      id,
      estado,
      observaciones
    );
